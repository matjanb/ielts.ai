...
export async function getProgressData(userId: string) {
  const supabase = db()
  const [bandHistory, writingSubmissions, speakingSubmissions, attempts, studySessions, profile] =
    await Promise.all([
      ...
      supabase.from('speaking_submissions').select('id,part,band_score,fluency_score,lexical_score,grammar_score,pronunciation_score,created_at').eq('user_id', userId).order('created_at', { ascending: false }).limit(20),
      ...
      supabase.from('study_sessions').select('skill,duration_minutes,created_at').eq('user_id', userId).order('created_at', { ascending: false }).limit(500),
      supabase.from('profiles').select('target_band_score').eq('id', userId).maybeSingle(),
    ])

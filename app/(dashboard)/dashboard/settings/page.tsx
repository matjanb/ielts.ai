  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">{t('dashboard.settingsPage.subtitle')}</p>
      </div>

      {/* Profile */}
      <section className="p-6 rounded-2xl bg-white dark:bg-gray-900/60 border border-gray-100 dark:border-gray-800">
        <h2 className="text-sm font-semibold text-gray-900 dark:text-white mb-4">{t('dashboard.settingsPage.profile')}</h2>
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t('dashboard.settingsPage.fullName')}</label>
            <input
              type="text"
              value={fullName}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t('dashboard.settingsPage.email')}</label>
            <input
              type="email"
              value={email}
              disabled
          ...
          >
            {saving
              ...
              : saved
              ? t('dashboard.settingsPage.saved')
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <span className="text-sm text-gray-700 dark:text-gray-300">{t('dashboard.settingsPage.currentPlan')}</span>
            </div>
          ...
          >
            {subscription === 'free' ? t('dashboard.settingsPage.upgrade') : t('dashboard.settingsPage.manage')}
      ...
      {/* Appearance */}
      <section className="p-6 rounded-2xl bg-white dark:bg-gray-900/60 border border-gray-100 dark:border-gray-800">
        <h2 className="text-sm font-semibold text-gray-900 dark:text-white mb-4">{t('dashboard.settingsPage.appearance')}</h2>
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm text-gray-700 dark:text-gray-300">{t('dashboard.settingsPage.theme')}</div>
              <div className="text-xs text-gray-400 dark:text-gray-500">{t('dashboard.settingsPage.themeDesc')}</div>
            </div>
            <ThemeToggle />
          </div>
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm text-gray-700 dark:text-gray-300">{t('dashboard.settingsPage.language')}</div>
              <div className="text-xs text-gray-400 dark:text-gray-500">{t('dashboard.settingsPage.languageDesc')}</div>
            </div>
            <LanguageSwitcher />
          </div>
        </div>
